...
def calculate_baseline_accuracy(y_train: np.ndarray) -> Dict[str, float]:
    ...
    baseline_metrics = {
        ...
        'class_distribution': dict(zip([int(u) for u in unique],
...
def plot_confusion_matrix(
    ...
    if save_path is None:
        save_path = RESULTS_DIR / f"{model_name.lower().replace(' ', '_')}_confusion_matrix.png"
...
def plot_roc_curve(
    ...
    """
    if save_path is None:
        save_path = RESULTS_DIR / f"{model_name.lower().replace(' ', '_')}_roc_curve.png"
...
def create_evaluation_report(
    ...
    if save_visualizations:
        ...
        if y_pred_proba is not None:
            viz_paths['roc_curve'] = plot_roc_curve(
