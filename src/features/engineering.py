...
def fit_pca_with_permutation_test(
    ...
    logger.info(
        f"Explained variance by first {N_PCA_COMPONENTS} components: "
...
def create_pca_features(
    data: pd.DataFrame, run_permutation_test: bool = True, save_output: bool = True
) -> pd.DataFrame:
    """
    ...
    feature_names = [
        col for col in data.columns if col not in EXCLUDE_COLUMNS and col != "result"
    ]
