...
The work started in Jupyter Notebooks for exploration and prototyping, then evolved into a modular Python project, including experiment tracking, automated pipelines, and comprehensive model evaluation.
...
Vision dominance is the second most important factor. High vision scores, ward placement/clearing, and control ward investment indicate map control. `gamelength` appears here because longer games amplify vision metrics and teams that maintain vision control in extended games typically win.
...
**Note:** PC4, PC5, and PC6 all heavily load on herald/grubs, indicating these objectives appear in multiple orthogonal dimensions of the data. This redundancy suggests that **early neutral objective control is multifaceted**: timing (first herald), total count, and denying opponents all contribute independently to match outcomes. The introduction of grubs this season added an influential and important objective for teams to play around.
...
1. **Gold > Everything Else**: PC1 (gold advantage) being the strongest component confirms that gold wins games. Economic leads at 20-25 minutes are the single best predictor of victory.

2. **Vision is Power**: PC2 demonstrates that vision control is nearly as important as gold. Professional teams that dominate vision create opportunities for objective control and avoid unfavorable fights. Vision is information, and information is power.
